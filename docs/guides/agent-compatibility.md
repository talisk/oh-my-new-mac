...
- 全新 arm64 Mac：没有 Brew/语言环境，展示分类工具清单；用户未全选时不默认装全套，明确全选后按选择安装。
...
宿主支持分组多选时按分类显示各工具；不支持时先在普通消息中展示五类的完整工具列表，再用其真实 schema 收集“全选自动 / 分类或名单 / 仅检查”等入口。不要虚构 multiSelect 参数或改变宿主模式来让工具可调用。

名单/冲突选择来自核心 skill，不来自 slash 入口。用户已用自然语言给出白名单、黑名单或 auto 时直接复用，不因改用其他 agent 又问一次。无工具但已明确授权的部分可继续；只有尚缺必要选择的部分需要对话回复。
